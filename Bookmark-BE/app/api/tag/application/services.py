...
class TagAppServices:
    ...
    async def create_tag(
        ...
        existing = await self.tag_domain_services.get_tag_by_name_and_user(
            current_user.id, tag_data.name.strip()
        )
        if existing:
        ...
        data = TagDataClass(user_id=current_user.id, name=tag_data.name.strip())
        tag = await self.tag_domain_services.create_tag(data)
        return {
            "id": tag.id,
            "name": tag.name,
            "created_at": tag.created_at,
